...
(function (factory) {
  ...
  function ChartJsProvider () {
    ...
    this.setOptions = function (type, customOptions) {
      ...
      if (! customOptions) {
        customOptions = type;
        //FIX BONITA options = angular.merge(options, customOptions);
        options = _.merge(options, customOptions);
      } else {
        // Set options for the specific chart
        //FIX BONITA options[type] = angular.merge(options[type] || {}, customOptions);
          options[type] = _.merge(options[type] || {}, customOptions);
      }

      //FIX BONITA angular.merge(ChartJs.Chart.defaults, options);
        _.merge(ChartJs.Chart.defaults, options);
    };
  ...
  function ChartJsFactory (ChartJs, $timeout) {
      ...
      return {
        ...
        datasets: data.map(function (item, i) {
          ...
          if (datasetOverride && datasetOverride.length >= i) {
            //FIX BONITA angular.merge(dataset, datasetOverride[i]);
              _.merge(dataset, datasetOverride[i]);
          }
          return dataset;
        })
      };
    ...
    function getData (labels, data, colors, datasetOverride) {
      ...
      if (datasetOverride) {
        //FIX BONITA angular.merge(dataset.datasets[0], datasetOverride);
          _.merge(dataset.datasets[0], datasetOverride);
      }
      return dataset;
    }
